...
class BoneSlicesDatasetPrev(Dataset):
    """
    ...
    def __init__(self, json_config_filepath: str = 'data_utils/config.json', transform = None, permute_order: List = [2,0,1]) -> None:
        """
        ...
        self.x_axis_metadata = self.get_metadata("x")
        self.y_axis_metadata = self.get_metadata("y")
        self.z_axis_metadata = self.get_metadata("z")
        self.metadata = pd.concat([self.x_axis_metadata, self.y_axis_metadata, self.z_axis_metadata])
    ...
    def __getitem__(self, idx: Union[int, ArrayLike]) -> Tuple[torch.tensor, float]:
        ...
        slice_idx = self.metadata.loc[idx, 'Slice Index']
        growth_plate_idx = self.metadata.loc[idx, 'Growth Plate Index']
    ...
    def get_metadata(self, axis: str) -> pd.DataFrame:
        """
        Gets metadata for a specified axis.
        Args:
            axis (str): Axis to retrieve ('x', 'y', or 'z').

        Returns:
            pd.DataFrame: Metadata DataFrame or None.
        """
        if self.config[f'{axis}-view']:
            df =  pd.read_csv(os.path.join(self.config['metadata_dir'], self.config[f'{axis}_axis_csv']))
            df["axis"] = axis
            # df['img_path'] = [os.path.join(self.config['data_dir'], f"{axis}_axis", img_name.split('.')[0] + '.npy') for img_name in df['img_file_name']]
            df['img_path'] = [os.path.join(self.config['data_dir'], f"{axis}_axis", img_name) for img_name in df['img_file_name']]
            return df
        else:
            return None
