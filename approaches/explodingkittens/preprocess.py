...
from plane_predict.utils import (
    apply_bone_window,
    equalize_hist_16bit,
    normalize_image,
    pad_crop_image,
...
class BoneProcessor:
    def __init__(self, metadata: pd.DataFrame, output_dir: str = "2_5D_images", size: tuple[int] = (512,512), bit_depth: int = 16, num_jobs: int = -1, offset_step: int = 5, apply_histogram_equalization: bool = False, window: tuple[int] = None):
        self.file_info = zip(metadata["image_path"].values, metadata["Image Name"].values, metadata["Growth Plate Index"].values)
    ...
    def process_scan(self, image):
        processed_scans = []
        metadata = []
        center_x, center_y, center_z = [dim // 2 for dim in image.shape]
            ...
            if 0 <= new_x < image.shape[0]:
                ...
                for offset_y in range(start_slice, end_slice, self.offset_step):
                    ...
                    if 0 <= new_y < image.shape[1]:
                        ...
                        combined_slice = normalize_image((0.5*x_slice + 0.5*y_slice), self.bit_depth)
                        processed_scans.append(np.stack([x_slice, y_slice, combined_slice], axis=-1))
    ...
    def process_files(self):
        ...
        Parallel(n_jobs=self.num_jobs)(
            delayed(self.process_and_save_images)(file_path, file_idx, gp_idx) for file_path, file_idx, gp_idx in tqdm(self.file_info)
        )
    ...
    def process_and_save_images(self, file_path, file_idx, gp_idx):
        ...
        for i, slice_image in enumerate(processed_slices):
            ...
            cv2.imwrite(f'{self.output_dir}/{file_idx}_slice_x{pos_x}_y{pos_y}_gp_{gp_idx}.png', slice_image)
