...
# Set image paths
BASE_PATH = Path("/home/ec2-user/SageMaker/shared/workstream-2")
IMAGES_PATH = BASE_PATH / "images_3d"
METADATA_PATH = BASE_PATH / "train.csv"

LABEL_REGEX = r"gp_([\d]{3})"
IMAGE_DIR = "./2_5D_images8bit"


def show_slices(slices, plate_idx = None):
    """ Function to display row of image slices """
    fig, axes = plt.subplots(1, len(slices), figsize=(10,7))
...
def get_metadata():
    ...
    metadata_df["image_path"] = metadata_df["Image Name"].map(lambda x: Path(f"{IMAGES_PATH}/{x}.nii"))
